...
const HPKE_BACKEND: &str = "rust-hpke (ohttp vendor)";
...
impl Server {
    ...
    pub fn decapsulate(&self, enc_request: &[u8]) -> Res<(Vec<u8>, ServerResponse)> {
        ...
        let (mut hpke, enc, info) = self.decode_request_header(&mut r, INFO_REQUEST)?;

        let pos = usize::try_from(r.position())?;
        let ct = &enc_request[pos..];
        debug!(
            "decap header: key_id={} kem={} kdf={} aead={} enc_len={} ct_len={} total={} info_hex={} backend={}",
            self.config.key_id,
            u16::from(hpke.config().kem()),
            u16::from(hpke.config().kdf()),
            u16::from(hpke.config().aead()),
            enc.len(),
            ct.len(),
            enc_request.len(),
            hex::encode(&info),
        ...
        debug!(
            "decap aead lens: key={} nonce={} tag={} enc_hex={} ct_hex={}",
            hpke.config().aead().n_k(),
            hpke.config().aead().n_n(),
            hpke.config().aead().n_t(),
            hex::encode(&enc),
            hex_trunc(ct, 256),
        );

        // Log exporter for response label prior to open.
        match export_secret(&hpke, LABEL_RESPONSE, hpke.config()) {
            Ok(secret) => {
                debug!(
                    "decap exporter LABEL_RESPONSE len={} hex={}",
                    secret.as_ref().len(),
                    hex::encode(secret.as_ref())
                );
            }
            Err(e) => debug!("decap exporter LABEL_RESPONSE failed: {}", e),
        }

        let request = match hpke.open(&[], ct) {
            Ok(v) => v,
            Err(e) => {
                debug!("decap hpke.open failed: {}", e);
                return Err(Error::from(e));
            }
        };
        debug!(
            "decap plaintext len={} hex={}",
            request.len(),
            hex_trunc(&request, 256)
        );
...
fn export_secret<E: Exporter>(exp: &E, label: &[u8], cfg: HpkeConfig) -> Res<SymKey> {
    let secret = exp.export(label, entropy(cfg))?;
    debug!(
        "export_secret: label={} len={} secret_hex={}",
...
fn make_aead(mode: Mode, cfg: HpkeConfig, secret: &SymKey, enc: &[u8], nonce: &[u8]) -> Res<Aead> {
    ...
    debug!(
        "make_aead: mode={:?} kdf={} aead={} salt_hex={} secret_hex={} prk_hex={} key_hex={} nonce_hex={}",
        mode,
        u16::from(cfg.kdf()),
        u16::from(cfg.aead()),
        hex::encode(&salt),
