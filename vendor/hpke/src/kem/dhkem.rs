    ...
    (
        ...
        pub(crate) mod $mod_name {
            use crate::{
                ...
                Deserializable, HpkeError, Serializable, hpke_debug_enabled,
            };
            ...
            impl KemTrait for $kem_name {
                ...
                fn decap(
                    ...
                    if hpke_debug_enabled() {
                        debug!(
                            "kem decap: pk_recip={} enc={} dh_eph={} suite_id={}",
                            hex::encode(pk_recip.to_bytes()),
                            hex::encode(encapped_key.to_bytes()),
                            hex::encode(kex_res_eph.to_bytes()),
                            hex::encode(&suite_id),
                        );
                    }
                    ...
                    if let Some(pk_sender_id) = pk_sender_id {
                        ...
                        if hpke_debug_enabled() {
                            debug!(
                                "kem decap authed: kem_context={} concatted_secrets={} shared_secret={}",
                                hex::encode(kem_context),
                                hex::encode(concatted_secrets),
                                hex::encode(shared_secret.0.as_slice()),
                            );
                        }
                        Ok(shared_secret)
                    } else {
                        ...
                        if hpke_debug_enabled() {
                            debug!(
                                "kem decap base: kem_context={} dh_eph={} shared_secret={}",
                                hex::encode(kem_context),
                                hex::encode(kex_res_eph.to_bytes()),
                                hex::encode(shared_secret.0.as_slice()),
                            );
