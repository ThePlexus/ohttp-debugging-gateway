...
pub fn labeled_extract<Kdf: KdfTrait>(
    ...
    if hpke_debug_enabled() {
        debug!(
            "HPKE LabeledExtract: salt_len={} salt_hex={} suite_id={} label='{}' label_hex={} ikm_len={} ikm_hex={}",
            salt.len(),
            hex::encode(salt),
            hex::encode(suite_id),
        ...
        let concat = [VERSION_LABEL, suite_id, label, ikm].concat();
        debug!(
            "HPKE LabeledExtract concat(VERSION_LABEL||suite_id||label||ikm) hex={}",
            hex::encode(concat)
        );
    }
    ...
    let res = extract_ctx.finalize();
    if hpke_debug_enabled() {
        debug!("HPKE LabeledExtract prk={}", hex::encode(res.0.as_slice()));
    }
    res
}
