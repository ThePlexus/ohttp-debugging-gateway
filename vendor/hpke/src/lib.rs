...
#[cfg(feature = "std")]
static HPKE_DEBUG: std::sync::OnceLock<std::sync::atomic::AtomicBool> =
    std::sync::OnceLock::new();

#[cfg(feature = "std")]
pub fn hpke_debug_enabled() -> bool {
    use std::sync::atomic::Ordering;
    let flag = HPKE_DEBUG.get_or_init(|| {
        let enabled = std::env::var("HPKE_DEBUG")
