    ...
    O: OpMode<Kem>,
{
    ...
    let (sched_context_buf, sched_context_size) = {
        ...
        if hpke_debug_enabled() {
            debug!(
                "hpke derive: shared_secret={} psk_id_hash={} info_hash={}",
                hex_trunc(shared_secret.0.as_slice(), 256),
                hex::encode(psk_id_hash.as_slice()),
    ...
    if hpke_debug_enabled() {
        debug!(
            "hpke derive: suite_id={} psk_len={} sched_ctx_len={} info_hex={}",
            hex::encode(&suite_id),
            mode.get_psk_bytes().len(),
            sched_context.len(),
            hex_trunc(info, 256),
        );
    }
    ...
    if hpke_debug_enabled() {
        debug!(
            "hpke derive result: key={} base_nonce={} exporter_secret={}",
            hex::encode(&key.0),
            hex::encode(&base_nonce.0),
            hex::encode(&exporter_secret.0)
        );
    }
